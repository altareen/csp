...
+ **Total Points:** 10
+ Implement a `Python` program that determines which gene is present in a DNA sequence.
...
## Testing
+ Run the file `testgenedetection.py` to execute the `PyUnit` test bench.
`PyUnit` indicates a successful test with an `OK` output, and an unsuccessful
test with a `FAIL` output.

## Submission
+ Upload the file `genedetection.py` to the [Web-CAT](http://ec2-54-65-207-33.ap-northeast-1.compute.amazonaws.com:8080/Web-CAT/WebObjects/Web-CAT.woa) automated grading platform.
